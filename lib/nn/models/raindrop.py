...
from ..layers import RaindropLayer
...
class Raindrop(nn.Module):
    def __init__(self,
        ...
        self.raindrop = RaindropLayer(input_size=self.d_in,
    ...
    def forward(self, x, mask=None, u=None, **kwargs):
        ...
        representation, states, h_enc, output = self.raindrop(x, self.adj, mask=mask, u=u,
                                                                                       cached_support=self.training )
        predictions = representation
        if self.training:
            return predictions, states, output, output
        return predictions, output, output
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument('--layer-norm', type=str_to_bool,
                            nargs='?', const=True, default=False)
        parser.add_argument('--global-att', type=str_to_bool,
                            nargs='?', const=True, default=False)
