...
from ..layers import BiGRIL, DGRIL
from ...utils.parser_utils import str_to_bool


class DGRINet(nn.Module):
    def __init__(self,
                 adj,
                 d_in,
                 d_hidden,
        ...
        self.dgrill = DGRIL(input_size=self.d_in,
                              ff_size=d_ff,
    ...
    def forward(self, x, mask=None, u=None, **kwargs):
        ...
        imputation, prediction, adj_dis = self.dgrill(x, self.adj, mask=mask, u=u, cached_support=self.training)
        ...
        if self.training:
            return imputation, prediction, adj_dis
        return imputation
