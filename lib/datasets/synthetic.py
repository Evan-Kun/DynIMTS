...
def generate_mask(shape, p_block=0.1, p_point=0.8, is_point=True, is_block=True, max_seq=1, min_seq=1, rng=None):
    """Generate mask in which 1 denotes valid values, 0 missing ones. shale=(steps, ...)"""
    if rng is None:
        rand = np.random.random
        randint = np.random.randint
    else:
    ...
    # let values before and after block missing always valid
        mask = np.where(rand(shape) > p_point, mask, 1 - mask)

    return mask


class SyntheticDataset(PandasDataset):
    # SEED: int
    ...
    def __init__(self,
        ...
        path = os.path.join(datasets_path['synthetic'], 'syntheic_data.npz')
        #fetchdata
        # filename = '..\..\datasets\synthetic\syntheic_data.npz'
        # filename = '../../datasets/synthetic/syntheic_data.npz'
        content = self.load(path)
        self.window = window if window is not None else content['loc'].shape[1]
        self.loc = torch.tensor(content['col'][:, :self.window]).float()
        self.adj = content['adj']
        self.seed = content['seed'].item()
        ...
        self.rng = np.random.default_rng(self.seed)
        # mask_shape = (len(self), self.window, self.n_nodes, 1)
        mask_shape = (self.n_nodes, self.window)
        mask = generate_mask(mask_shape,
                             p_block=p_block,
                             p_point=p_point,
                             max_seq=max_seq,
    ...
    # def train_dataloader(self, shuffle=False, batch_size=32):
    #     return Dataloader(Subset(self, self.test_idxs), shuffle=shuffle, batch_size=batch_size, drop_last = True)
    #
    # def val_dataloader(self, shuffle= False, batch_size=32):
    #     return Dataloader(Subset(self, self.val_idxs), shuffle=shuffle, batch_size=batch_size)
    #
