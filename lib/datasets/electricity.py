...
class Electricity(PandasDataset):
    def __init__(self, impute_nans=False, freq='1H', masked_sensors=None):
        self.eval_mask = None
        ...
        self.dist = dist
        super().__init__(dataframe=df, u=None, mask=mask, name='electricity', freq=freq, aggr='nearest')


    def load_raw(self):
        path = os.path.join(datasets_path['electricity'], 'electricity.npz')
        eval_mask = None
        content = np.load(path)
        df = pd.DataFrame(content['col'])
        periods = len(df)
        date = pd.date_range(start='1/1/2012', periods=periods, freq='H')
        df['date'] = date
...
class MissingElectricity(Electricity):

    def __init__(self, missing_ratio=0.8, seed=1):
        super(MissingElectricity, self).__init__()
