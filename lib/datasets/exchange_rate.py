...
class ExchangeRate(PandasDataset):
    def __init__(self, impute_nans=False, freq='1D', masked_sensors=None):
        self.eval_mask = None
        ...
        self.dist = dist
        super().__init__(dataframe=df, u=None, mask=mask, name='exchange_rate', freq=freq, aggr='nearest')


    def load_raw(self):
        path = os.path.join(datasets_path['exchange_rate'], 'exchange_rate.npz')
        eval_mask = None
        content = np.load(path)
        df = pd.DataFrame(content['col'])
        periods = len(df)
        date = pd.date_range(start='1/1/1990', periods=periods, freq='D')
        df['date'] = date
...
class MissingExchangeRate(ExchangeRate):

    def __init__(self, missing_ratio=0.8, seed=1):
        super(MissingExchangeRate, self).__init__()
