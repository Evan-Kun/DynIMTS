...
class AirQuality(PandasDataset):
    ...
    def load(self, impute_nans=True, small=False, masked_sensors=None):
        ...
        missing_mask = np.random.rand(eval_mask.shape[0], eval_mask.shape[1])
        missing_ratio = 0.7
        missing_ratio_mask = missing_mask < missing_ratio
        eval_mask_missing = np.logical_xor(missing_ratio_mask, eval_mask)
        eval_mask_missing = eval_mask_missing.astype('uint8')
        ...
        # self.eval_mask = eval_mask_missing.to_numpy()
