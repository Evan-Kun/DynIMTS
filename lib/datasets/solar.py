...
class Solar(PandasDataset):
    def __init__(self, impute_nans=False, freq='60T', masked_sensors=None):
        self.eval_mask = None
        ...
        self.dist = dist
        super().__init__(dataframe=df, u=None, mask=mask, name='solar', freq=freq, aggr='nearest')


    def load_raw(self):
        path = os.path.join(datasets_path['solar'], 'solar.npz')
        eval_mask = None
        content = np.load(path)
        df = pd.DataFrame(content['col'])
        periods = len(df)
        date = pd.date_range(start='1/1/2006', periods=periods, freq='10T')
        df['date'] = date
...
class MissingSolar(Solar):

    def __init__(self, missing_ratio=0.8, seed=1):
        super(MissingSolar, self).__init__()
